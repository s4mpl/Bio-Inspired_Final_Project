...
def get_points(center, radius, angle):
...
class Simple_Robot:
    ...
    def render(self, surface):
        points = get_points([10 * self.x + 960, 10 * self.y + 540], 25, self.a)
        draw.polygon(surface, "green", points)

    def update(self, coords, angle, dt):
        # get new x and y value from model / robot odom
        output = self.net.activate(self.get_status(coords[0], coords[1], angle))
        self.motor_drive(output, dt)


class Spin_Robot:
    def __init__(self, x, y, a, file="demo_files\\neat-checkpoint-99"):
        self.x = x
        self.y = y
        self.v = [0, 0, 0, 0]
        self.a = (rand.random() * 2 * math.pi) if a == -1 else a
        self.radius = 0.5
        check = neat.Checkpointer.restore_checkpoint(file)
        winner = check.run(eval_genomes2, 1)
        config = neat.Config(
            neat.DefaultGenome,
            neat.DefaultReproduction,
            neat.DefaultSpeciesSet,
    ...
    def render(self, surface):
        points = get_points([10 * self.x + 960, 10 * self.y + 540], 25, self.a)
        draw.polygon(surface, "blue", points)
